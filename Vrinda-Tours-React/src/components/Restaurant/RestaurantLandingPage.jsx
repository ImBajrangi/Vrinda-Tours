...
import { 
  UtensilsCrossed, Shield, CheckCircle2, ChevronRight, ChevronDown, Phone, User, 
  ...
  Mail, Lock, Eye, EyeOff, Compass, Cake, Coffee, Soup, Store, Car, Building2,
  Share2, Copy
} from 'lucide-react';
...
export const RestaurantGraphic = ({ type = '', size = 22 }) => {
  if (type.includes('Sweet') || type.includes('Peda')) return <Cake size={size} />;
  if (type.includes('Cafe') || type.includes('Prasad') || type.includes('Snacks')) return <Coffee size={size} />;
  if (type.includes('Heritage') || type.includes('Dining')) return <UtensilsCrossed size={size} />;
  return <Soup size={size} />;
};

export const RESTAURANT_TYPES = [
  {
    id: 'Sattvic Bhojnalaya',
    label: 'Pure Sattvic Bhojnalaya',
    desc: '100% No Onion/Garlic, traditional thali & kadhi-chawal',
    category: 'SATTVIC THALI',
    earningEst: '₹3,500–₹9,000 / day',
    tripRate: '40–80 Devotee Thalis / Day'
  },
  {
    id: 'Braj Sweets & Peda Outlet',
    label: 'Braj Sweets & Peda',
    desc: 'Authentic Mathura Peda, Rabdi, Makhan Mishri & Lassi',
    category: 'SWEETS & PRASAD',
    earningEst: '₹4,000–₹12,000 / day',
    tripRate: '60–120 Orders & Boxes / Day'
  },
  {
    id: 'Heritage Pilgrim Dining',
    label: 'Heritage Dining & AC Hall',
    desc: 'Chappan Bhog thalis, North/South Indian & AC family seating',
    category: 'FAMILY DINING',
    earningEst: '₹5,000–₹15,000 / day',
    tripRate: '80–150 Visiting Guests / Day'
  },
  {
    id: 'Temple Prasad Cafe',
    label: 'Temple Prasad & Snacks',
    desc: 'Bedmi Puri, Kachori, Jalebi, Chai & quick pilgrim snacks',
    category: 'QUICK PRASAD',
    earningEst: '₹2,500–₹7,500 / day',
    tripRate: '100+ Pilgrim Snack Orders / Day'
  }
];

export const RESTAURANT_SPECIALITIES = [
  '100% No Onion & Garlic',
  'Mathura Peda & Rabdi',
  ...
  'Temple Delivery Box',
  'AC Family Seating',
  'Bulk Group Yatra Thali'
];

export const SEATING_CAPACITIES = ['Under 20 Seats', '20–50 Seats', '50–100 Seats', '100+ Seats'];
...
export default function RestaurantLandingPage({ 
  onClose, 
  onOpenRestaurantCompanion,
  onOpenDriverPage,
  onOpenHotelPage,
  onOpenAgencyPage
}) {
  const [activeView, setActiveView] = useState('landing'); // 'landing' | 'wizard'
  const [currentStep, setCurrentStep] = useState(1); // 1: Personal/Auth, 2: Cuisine & Setup, 3: Zone & Confirm
  const [showLoginModal, setShowLoginModal] = useState(false);
  const [regSessionId] = useState(() => 'rreg_' + Date.now() + '_' + Math.random().toString(36).substring(2, 7));
  ...
  const [regForm, setRegForm] = useState({
    ...
    password: '',
    phone: '',
    restaurantType: 'Sattvic Bhojnalaya',
    seatingCapacity: '20–50 Seats',
    restaurantSpecialities: ['100% No Onion & Garlic', 'Mathura Peda & Rabdi', '100% Pure Desi Ghee'],
  ...
  const handleShareCurrentPage = async () => {
    ...
    const shareUrl = `${window.location.origin}/?join=dining&mode=register${refCode ? `&ref=${refCode}` : ''}`;
    const shareText = `Radhe Radhe! 🍽️ Partner your Bhojnalaya or Sweets shop on Vrinda Vihar with 0% listing fee & direct pilgrim orders:\n\n${shareUrl}`;
    
    if (navigator.share) {
      try {
        await navigator.share({ title: 'Dining & Bhojnalaya Desk - Vrinda Vihar', text: shareText, url: shareUrl });
  ...
  useEffect(() => {
    const handleUrlState = () => {
      try {
        ...
        if (sectionId && sectionId !== 'restaurant' && sectionId !== 'dining' && sectionId !== 'food' && sectionId !== 'landing') {
  ...
  const selectedRestaurantObj = RESTAURANT_TYPES.find(r => r.id === regForm.restaurantType) || RESTAURANT_TYPES[0];

  const toggleSpeciality = (spec) => {
    setRegForm(prev => {
      const currentList = prev.restaurantSpecialities || [];
      const exists = currentList.includes(spec);
      const updatedList = exists ? currentList.filter(i => i !== spec) : [...currentList, spec];
      return { ...prev, restaurantSpecialities: updatedList };
    });
  };
  ...
  useEffect(() => {
    ...
    const checkGoogleAuthReturn = async () => {
      try {
        const rawPending = sessionStorage.getItem('vt_pending_restaurant_reg');
        if (!rawPending) return;

        const { data: { session } } = await supabase.auth.getSession();
        
        if (session?.user && isMounted) {
          sessionStorage.removeItem('vt_pending_restaurant_reg');

          const user = session.user;
          const googleName = user.user_metadata?.full_name || user.user_metadata?.name || 'Restaurant Partner';
          ...
          if (pendingData.intent === 'step1_google_login' || !pendingData.completed) {
            ...
            try {
              await supabase.from('restaurant_registrations').upsert({
                id: pendingData.regSessionId || ('rreg_' + Date.now()),
                step: 1,
                outlet_name: googleName,
              ...
              console.warn('Restaurant registration Supabase sync fallback:', syncErr);
            }
            return;
          }
        }
  ...
  const syncProgressToSupabase = async (stepNumber, additionalData = {}) => {
    try {
      ...
      const payload = {
        id: regSessionId,
        step: stepNumber,
        outlet_name: regForm.name.trim() || null,
        owner_name: regForm.name.trim() || null,
        phone: cleanPhone || null,
        email: regForm.email.trim() || null,
        cuisine_type: regForm.restaurantType,
        seating_capacity: regForm.seatingCapacity,
        specialities: regForm.restaurantSpecialities || [],
      ...
      try {
        localStorage.setItem('vt_restaurant_reg_' + regSessionId, JSON.stringify(payload));
      } catch (e) {}

      const { error } = await supabase.from('restaurant_registrations').upsert(payload);
      if (error) {
        if (error.status !== 403 && error.code !== '42P01') {
          console.warn('restaurant_registrations sync step ' + stepNumber + ' notice:', error.message);
        }
      }
    } catch (err) {
      console.warn('restaurant_registrations sync fallback:', err?.message || err);
    }
  };
  ...
  const handleGoogleSignInStep1 = async () => {
    ...
    try {
      sessionStorage.setItem('vt_pending_restaurant_reg', JSON.stringify({
        regSessionId,
        phone: regForm.phone,
        restaurantType: regForm.restaurantType,
        seatingCapacity: regForm.seatingCapacity,
        zone: regForm.zone,
  ...
  const handleStep1Next = async (e) => {
    ...
    if (!regForm.name.trim() || regForm.name.trim().length < 2) {
      setRegError('Please enter restaurant or owner legal name');
      return;
    }

    if (!regForm.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(regForm.email.trim())) {
      setRegError('Please enter a valid email address for your dining dashboard');
      return;
    }
  ...
  const handleFinalSubmit = async (e) => {
    ...
    try {
      const phoneValidation = validatePhoneNumber(regForm.phone);
      const cleanPhone = phoneValidation.clean || regForm.phone;
      const newRestaurantId = 'r_' + Date.now();
      ...
      try {
        await supabase.from('partners').upsert({
          ...
          email: regForm.email.trim(),
          category: 'restaurant',
          role_details: `🍽️ ${regForm.restaurantType} • ${regForm.seatingCapacity} • ${regForm.zone.split(' ')[0]}`,
          rating: 5.0,
          status: 'Pending Admin Verification',
          verified: false,
          category_locked: false,
          photo_url: `https://images.unsplash.com/photo-1517248135467-4c7ed9d8c47c?w=400`,
          metadata: {
            cuisineType: regForm.restaurantType,
            seatingCapacity: regForm.seatingCapacity,
            specialities: regForm.restaurantSpecialities,
            zone: regForm.zone,
            email: regForm.email.trim()
      ...
      sessionStorage.setItem('vt_partner_id', newRestaurantId);
      sessionStorage.setItem('vt_partner_role', 'restaurant');
      
      setRegSuccess(true);
      setTimeout(() => {
        if (onOpenRestaurantCompanion) {
          onOpenRestaurantCompanion(newRestaurantId, 'restaurant');
        }
      }, 1200);
  ...
  const handleRestaurantLogin = async (e) => {
    ...
    if (loginMode === 'email') {
      ...
      try {
        ...
        if (error) throw error;
        const rid = data.user?.id ? `r_${data.user.id.slice(0, 8)}` : `r_${Date.now()}`;
        sessionStorage.setItem('vt_partner_id', rid);
        sessionStorage.setItem('vt_partner_role', 'restaurant');
        setIsLoggingIn(false);
        if (onOpenRestaurantCompanion) {
          onOpenRestaurantCompanion(rid, 'restaurant');
        }
      } catch (err) {
    ...
    const restaurantId = `rest_${cleanNumber}`;
    sessionStorage.setItem('vt_partner_id', restaurantId);
    sessionStorage.setItem('vt_partner_role', 'restaurant');
    setIsLoggingIn(false);
    if (onOpenRestaurantCompanion) {
      onOpenRestaurantCompanion(restaurantId, 'restaurant');
    }
  };
          ...
          <div className="dmd-announcement-track">
            ...
            <div className="dmd-announcement-list">
              <div className="dmd-announcement-item" onClick={() => { setActiveView('wizard'); setCurrentStep(1); }}>
                <span>🍲 <strong>3,500+ Sattvic Dining Searches:</strong> Devotees looking for Pure Vaishnav &amp; No-Onion-No-Garlic dining</span>
                <span className="highlight-green">● Active Radar</span>
              </div>
              <span className="dmd-announcement-sep">✦</span>
              <div className="dmd-announcement-item" onClick={() => { setActiveView('wizard'); setCurrentStep(1); }}>
                <span>🍛 <strong>0% Food Commission:</strong> Devotees pay directly at your counter with ₹0 platform fees</span>
                <span className="dmd-announcement-action">0% Cut</span>
              </div>
              <span className="dmd-announcement-sep">✦</span>
              <div className="dmd-announcement-item" onClick={() => { setActiveView('wizard'); setCurrentStep(1); }}>
                <span>🍯 <strong>Mathura Peda &amp; Sweets Feature:</strong> Connect directly with visiting pilgrim families on Vrinda map</span>
                <span className="highlight-gold">Direct Counter</span>
              </div>
              <span className="dmd-announcement-sep">✦</span>
              <div className="dmd-announcement-item" onClick={() => { setActiveView('wizard'); setCurrentStep(1); }}>
                <span>🚌 <strong>Bulk Yatra Group Pre-Orders:</strong> 50+ passenger bus parikrama dining bookings delivered to WhatsApp</span>
              </div>
              <span className="dmd-announcement-sep">✦</span>
              <div className="dmd-announcement-item" onClick={() => { setActiveView('wizard'); setCurrentStep(1); }}>
                <span>✨ <strong>Free Bhojnalaya Listing:</strong> Showcase your prasad dining to devotees across India</span>
        ...
        <div className="dmd-nav-container">
          ...
          <div className="dmd-brand-block" ref={portalDropdownRef}>
            ...
            >
              ...
              <div className="dmd-brand-info">
                ...
                <span className="dmd-brand-category">Dining</span>
              </div>
          ...
          <nav className="dmd-nav-center">
            ...
            <a 
              ...
              onClick={(e) => { e.preventDefault(); scrollToSection('cuisines'); }}
            >
              Cuisines
            </a>
            <a 
          ...
          <div className="dmd-nav-right">
            <button 
              ...
              title="Share Dining Desk registration link"
      ...
      <main className="dmd-main-body" ref={mainBodyRef} onScroll={handleBodyScroll}>
        ...
        {activeView === 'wizard' ? (
              ...
              <div className="dmd-stepper-header">
                <div className="dmd-stepper-steps">
                  ...
                  <div className={`dmd-step-node ${currentStep >= 2 ? 'active' : ''}`}>
                    ...
                    <span className="dmd-step-name">Dining</span>
                  </div>
              ...
              <div className="dmd-wizard-grid">
                ...
                <div className="dmd-wizard-summary-col">
                  <div className="dmd-summary-card dmd-wow-card">
                    ...
                    {/* Live Restaurant ID Hologram */}
                    <div className="dmd-id-card-live">
                      <div className="dmd-id-card-top">
                        <span className="dmd-id-pill">PRE-APPROVED DINING</span>
                        <span className="dmd-id-num">#VT-842</span>
                      </div>
                      <div className="dmd-id-card-body">
                        <div className="dmd-id-avatar">
                          <RestaurantGraphic type={regForm.restaurantType} size={28} />
                        </div>
                        <div className="dmd-id-meta">
                          <strong>{regForm.name || 'Your Bhojnalaya Name'}</strong>
                          <span>{regForm.phone ? `+91 ${regForm.phone}` : '+91 Mobile Active'}</span>
                          <span className="dmd-id-vehicle-tag">{regForm.restaurantType} • {regForm.seatingCapacity}</span>
                        </div>
                      </div>
                    </div>

                    <div className="dmd-earning-projection-box">
                      <span className="dmd-proj-title">ESTIMATED ORDERS</span>
                      <div className="dmd-proj-amount">{selectedRestaurantObj.earningEst}</div>
                      <div className="dmd-proj-sub">{selectedRestaurantObj.tripRate}</div>
                    </div>
                    ...
                    <div className="dmd-summary-checklist">
                      ...
                      <div className="dmd-check-item">
                        <CheckCircle2 size={16} className="dmd-check-icon" />
                        <span>Live Pilgrim Dining Map</span>
                      </div>
                      <div className="dmd-check-item">
                        <CheckCircle2 size={16} className="dmd-check-icon" />
                        <span>Bulk Yatra Group Orders</span>
                      </div>
                    </div>
                ...
                <div className="dmd-wizard-form-col">
                    ...
                    <div className="dmd-step-card dmd-fade-in">
                      ...
                      <form onSubmit={handleStep1Next} className="dmd-step-form">
                        <div className="dmd-form-group">
                          <label className="dmd-label">Dining Outlet / Restaurant Name</label>
                          <div className="dmd-input-wrap">
                            <UtensilsCrossed size={18} className="dmd-input-icon" />
                            <input 
                              type="text" 
                              required 
                              placeholder="e.g. Shri Brijwasi Sattvic Bhojnalaya" 
                        ...
                        <div className="dmd-form-group">
                          ...
                          <div className="dmd-input-wrap">
                            ...
                            <input 
                              type="email" 
                              required 
                              placeholder="contact@brijwasidining.com" 
                              className="dmd-input dmd-input-large"
                              value={regForm.email}
                        ...
                        <div className="dmd-form-group">
                          <label className="dmd-label">Order Mobile / WhatsApp</label>
                          ...
                          <span className="dmd-input-hint">For table & thali pre-orders</span>
                        </div>
                  ...
                  {/* STEP 2: CUISINE & CAPACITY */}
                  {currentStep === 2 && (
                    <div className="dmd-step-card dmd-fade-in">
                      <div className="dmd-card-header">
                        <span className="dmd-card-eyebrow">STEP 2 OF 3</span>
                        <h2 className="dmd-step-title">Dining Details</h2>
                        <p className="dmd-step-subtitle">Select cuisine type and seating capacity.</p>
                      </div>

                      <form onSubmit={handleStep2Next} className="dmd-step-form">
                        
                        <div className="dmd-form-group">
                          <label className="dmd-label">Cuisine Category</label>
                          <div className="dmd-catalog-card-selector">
                            {RESTAURANT_TYPES.map(r => (
                              <div 
                                key={r.id} 
                                className={`dmd-catalog-option-card ${regForm.restaurantType === r.id ? 'active' : ''}`}
                                onClick={() => setRegForm({ ...regForm, restaurantType: r.id })}
                              >
                                <div className="dmd-v-card-top-row">
                                  <div className="dmd-v-card-icon">
                                    <RestaurantGraphic type={r.id} size={22} />
                                  </div>
                                  <span className="dmd-v-card-cat">{r.category}</span>
                                </div>
                                <div className="dmd-v-card-title">{r.label}</div>
                                <div className="dmd-v-card-desc">{r.desc}</div>
                                <div className="dmd-v-card-earning">{r.earningEst}</div>
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div className="dmd-form-group">
                          <label className="dmd-label">Seating Capacity</label>
                          <div className="dmd-chip-group">
                            {SEATING_CAPACITIES.map(sc => (
                              <button 
                                type="button" 
                                key={sc} 
                                className={`dmd-select-chip ${regForm.seatingCapacity === sc ? 'active' : ''}`}
                                onClick={() => setRegForm({ ...regForm, seatingCapacity: sc })}
                              >
                                {sc}
                              </button>
                            ))}
                        ...
                        <div className="dmd-form-group">
                          <label className="dmd-label">Specialties &amp; Guarantees</label>
                          <div className="dmd-multi-chip-group">
                            {RESTAURANT_SPECIALITIES.map(spec => {
                              const isSelected = (regForm.restaurantSpecialities || []).includes(spec);
                              return (
                                <button
                                  type="button"
                                  key={spec}
                  ...
                  {currentStep === 3 && (
                    <div className="dmd-step-card dmd-fade-in">
                      <div className="dmd-card-header">
                        ...
                        <p className="dmd-step-subtitle">Pick territory where your outlet is located.</p>
                      </div>
                      ...
                      <form onSubmit={handleFinalSubmit} className="dmd-step-form">
                        ...
                        <div className="dmd-zero-comm-callout">
                          ...
                          <div className="dmd-callout-text">
                            ...
                            <p>Keep 100% of all food and prasad billings directly from pilgrims. ₹0 fee.</p>
                          </div>
                        </div>
          ...
          <>
                ...
                <div className="dmd-hero-grid">
                  ...
                  <div className="dmd-hero-content">
                    <span className="dmd-eyebrow">
                      श्री धाम ब्रज सेवा • BRAJ DINING
                    </span>

                    <h1 className="dmd-display-lg">
                      Serve Sacred Braj Flavors. <br className="dmd-desktop-br" />Welcome Thousands of Devotees.
                    </h1>

                    <p className="dmd-lead-text">
                      Showcase Sattvic Bhojnalayas, sweet shops and prasad dining. Get featured on live pilgrim maps with zero commission.
                    </p>
                    ...
                    <div className="dmd-metrics-strip">
                      <div className="dmd-metric-box">
                        ...
                        <span className="dmd-metric-sub">Order Cut</span>
                      </div>
                      <div className="dmd-metric-box">
                        <span className="dmd-metric-num">100%</span>
                        <span className="dmd-metric-sub">Direct Payment</span>
                      </div>
                      <div className="dmd-metric-box">
                        <span className="dmd-metric-num">1.2k+</span>
                        <span className="dmd-metric-sub">Daily Foodies</span>
                      </div>
                    </div>
                  ...
                  <div className="dmd-hero-card-col">
                    <div className="dmd-card dmd-interactive-preview">
                      <div className="dmd-preview-header">
                        <span className="dmd-card-eyebrow">PRASAD &amp; DINING</span>
                        <span className="dmd-coming-soon-pill">
                          COMING SOON
                        </span>
                      </div>

                      <h2 className="dmd-card-title">Live Table &amp; Thali Bookings</h2>
                      <p className="dmd-card-desc">Feature on the live pilgrim map for hungry devotees.</p>

                      {/* Fogged / Blurred Dining Ticket */}
                        ...
                        <div className="dmd-dispatch-ticket dmd-fogged-ticket">
                          ...
                          <div className="dmd-route-stop">
                            ...
                            <div className="dmd-stop-details">
                              <div className="dmd-stop-name">Brijwasi Sattvic Bhojnalaya</div>
                              <div className="dmd-stop-sub">4x Pure Desi Ghee Special Thali + Mathura Peda</div>
                            </div>
                          </div>
                          ...
                          <div className="dmd-route-track">
                            ...
                            <div className="dmd-track-pill">
                              <span>Table Ready in 10 mins • Pre-Ordered</span>
                            </div>
                          </div>
                          ...
                          <div className="dmd-route-stop">
                            ...
                            <div className="dmd-stop-details">
                              <div className="dmd-stop-name">VIP Devotee Table Reservation</div>
                              <div className="dmd-stop-sub">Pure Sattvic • 100% No Onion / Garlic</div>
                            </div>
                          </div>
                          ...
                          <div className="dmd-ticket-fare-row">
                            <div className="dmd-fare-col">
                              <span className="dmd-fare-label">BILL AMOUNT</span>
                              <span className="dmd-fare-amount">₹960</span>
                            </div>
                            <div className="dmd-commission-badge">
                              <ShieldCheck size={14} />
                              <span>100% Restaurant Keeps</span>
                            </div>
                          </div>
                        </div>
            ...
            {/* SECTION 2: SUPPORTED CUISINES */}
            <section className="dmd-section" id="cuisines">
              <div className="dmd-container">
                <div className="dmd-section-intro">
                  <span className="dmd-eyebrow">CUISINE CATEGORIES</span>
                  <h2 className="dmd-display-md">Supported Dining &amp; Outlets</h2>
                  <p className="dmd-body">Whether you run a traditional sattvic bhojnalaya or a famous Mathura peda sweet shop, all outlets are welcome.</p>
                </div>
                ...
                <div className="dmd-fleet-grid">
                  {RESTAURANT_TYPES.map(r => (
                    <div 
                      ...
                      onClick={() => {
                        setRegForm(prev => ({ ...prev, restaurantType: r.id }));
                        setActiveView('wizard');
                        setCurrentStep(2);
                      }}
                    >
                      <div className="dmd-fleet-header">
                        <div className="dmd-fleet-icon-box">
                          <RestaurantGraphic type={r.id} size={24} />
                        </div>
                        <span className="dmd-category-badge">{r.category}</span>
                      </div>
                      <h3 className="dmd-fleet-title">{r.label}</h3>
                      <p className="dmd-fleet-desc">{r.desc}</p>
                      <div className="dmd-fleet-earning-pill">{r.earningEst}</div>
                      <div className="dmd-fleet-footer">
                        <span>Register this outlet</span>
            ...
            <section className="dmd-section" id="benefits">
              <div className="dmd-container">
                <div className="dmd-color-block-cream">
                  <span className="dmd-eyebrow">DINING BENEFITS</span>
                  <h2 className="dmd-display-md">Built for Sacred Braj Food Seva</h2>
                  <p className="dmd-body">Empowering local restaurants and sweet shops with direct devotee footfall and zero commission deductions.</p>
                  ...
                  <div className="dmd-pillars-grid">
                    <div className="dmd-pillar-card">
                      ...
                      <h3 className="dmd-pillar-title">0% Food Commission Forever</h3>
                      <p className="dmd-pillar-desc">
                        No 20–30% food aggregator cuts. You receive 100% of the customer bill directly into your till or personal UPI.
                      </p>
                    </div>
                    ...
                    <div className="dmd-pillar-card">
                      ...
                      <h3 className="dmd-pillar-title">Featured on Pilgrim Map</h3>
                      <p className="dmd-pillar-desc">
                        Your kitchen appears on the interactive darshan map with direct route directions for pilgrims seeking authentic prasad.
                      </p>
                    </div>
                    ...
                    <div className="dmd-pillar-card">
                      ...
                      <h3 className="dmd-pillar-title">Group Yatra Thali Pre-Orders</h3>
                      <p className="dmd-pillar-desc">
                        Receive bulk thali pre-orders from arriving bus groups, pilgrim families, and ashram visitors before meal times.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <section className="dmd-section" id="territories">
              <div className="dmd-container">
                <div className="dmd-section-intro">
                  ...
                  <p className="dmd-body">Select where your restaurant or sweet shop is located in Braj Dham.</p>
                </div>
            ...
            <section className="dmd-section" id="faq">
              <div className="dmd-container">
                <div className="dmd-section-intro">
                  ...
                  <h2 className="dmd-display-md">Clear Answers for Dining Outlets</h2>
                </div>

                <div className="dmd-faq-list">
                  {[
                    {
                      q: "Is registration free for sweet shops and small bhojnalayas?",
                      a: "Yes. Registration is 100% free with zero commission on all thali, sweets, and table reservations."
                    },
                    {
                      q: "Must our restaurant be purely vegetarian?",
                      a: "Yes. In accordance with Braj Dham sanctity, all registered dining outlets must serve 100% Pure Vegetarian / Sattvic food with No Onion / No Garlic options highlighted."
                    },
                    {
                      q: "How will we receive table reservations & bulk pre-orders?",
                      a: "Devotees reserve tables directly on Vrinda.Tours with instant WhatsApp notification sent to your manager's phone."
                    }
                  ].map((faq, idx) => (
                    <div 
                      key={idx} 
              ...
              <div className="dmd-container dmd-footer-inner">
                <div className="dmd-footer-brand-section">
                  <div className="dmd-footer-brand-header">
                    ...
                    <span className="dmd-brand-title">Vrinda. Dining</span>
                  </div>
        ...
        <div className="dmd-modal-backdrop">
          <div className="dmd-modal-dialog success">
            ...
            <p className="dmd-modal-desc">
              Your dining outlet has been registered with <strong>0% Commission Guaranteed</strong>.
            </p>
            <div className="dmd-id-card-success">
              <span className="dmd-id-pill">VERIFIED BRIJ DINING</span>
              <div className="dmd-id-name">{regForm.name}</div>
              <div className="dmd-id-phone">+91 {regForm.phone}</div>
              <div className="dmd-id-role">{regForm.restaurantType} • {regForm.zone}</div>
            </div>
            <button 
              type="button" 
              className="dmd-btn-primary dmd-btn-large dmd-btn-full"
              onClick={() => {
                setRegSuccess(false);
                if (onOpenRestaurantCompanion) {
                  onOpenRestaurantCompanion(sessionStorage.getItem('vt_partner_id'), 'restaurant');
                }
              }}
            >
              Open Dining Companion Portal
            </button>
          </div>
        </div>
      )}

      {/* RESTAURANT SIGN IN MODAL */}
      {showLoginModal && (
        <div className="dmd-modal-backdrop">
          <div className="dmd-modal-dialog">
            ...
            <div className="dmd-modal-header">
              <span className="dmd-card-eyebrow">DINING &amp; BHOJNALAYA ACCESS</span>
              <h2 className="dmd-modal-title">Sign In to Dining Portal</h2>
              <p className="dmd-modal-desc">Manage table reservations, thali orders, and devotee customer requests.</p>
            </div>
            ...
            <form onSubmit={handleRestaurantLogin} className="dmd-step-form">
              {loginMode === 'phone' ? (
                <div className="dmd-form-group">
                  ...
                  <div className="dmd-form-group">
                    ...
                    <div className="dmd-input-wrap">
                      ...
                      <input 
                        type="email" 
                        required 
                        placeholder="contact@bhojnalaya.com" 
                        className="dmd-input dmd-input-large"
                        value={loginEmail}
              ...
              >
                {isLoggingIn ? 'Verifying...' : 'Sign In to Dining Portal'}
              </button>
            </form>

            <div className="dmd-modal-footer-note">
              <span>New to Vrinda Dining? </span>
              <button 
                type="button" 
